...
if PyQT:
    ...
    from ipc.Game import ServerViewQt as ServerView
else:
...
class ServerController(QWidget):
    ...
    pCastle2 = QPalette(Qt.white)
    pGrass = QPalette(Qt.green)
    pForest = QPalette(Qt.darkGreen)
    pLake = QPalette(Qt.darkBlue)
    pMountain = QPalette(Qt.darkGray)
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                if i == self.player1[0] and j == self.player1[1]:
                    self.player1label = QLabel(widget)
                ...
                if i == self.player2[0] and j == self.player2[1]:
                    self.player2label = QLabel(widget)
